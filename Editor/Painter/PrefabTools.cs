...
namespace LcLTools
{
    public class PrefabTools : EditorWindow
    {
        ...
        [MenuItem("LcLTools/PrefabTools %g", false, 1000)]
        static void OpenWindow()
        {
            PrefabTools window = GetWindow<PrefabTools>();
        ...
        public void CreateGUI()
        {
            ...
            box = new VisualElement();
            box.AddToClassList("boxH");
            if (jsonField == null) jsonField = new TextField("Json保存路径") { value = "Resources/Render/Grass.json" };
            jsonField.AddToClassList("input-field");
            // var openButton = new Button() { text = "打开" };
            // openButton.RegisterCallback<ClickEvent>(OpenFolderPath);
            box.Add(jsonField);
            // box.Add(openButton);
            paramsBox.Add(box);
        ...
        // void OpenFolderPath(ClickEvent evt)
        // {
        ...
        public void SaveGrassData(ClickEvent evt)
        {
            ...
            var path = Application.dataPath + "/" + jsonField.value;
