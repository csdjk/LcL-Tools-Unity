...
namespace LcLTools
{
    ...
    public class SVNTools : Editor
    {
        ...
        // svn log
        [MenuItem("Assets/LcL SVN Operation/SVN Log...")]
        public static void Log()
        {
            var dir_path = LcLEditorUtilities.GetSelectionAssetPath();
            if (dir_path == "") return;
            string cmd = string.Format(GetCmdFormatString(), SVNCmd.LOG, dir_path);
            Debug.Log(cmd);
            InvokeCmd(cmd);
        }
        // svn revert
        [MenuItem("Assets/LcL SVN Operation/SVN Revert...")]
        public static void Revert()
        {
            var dir_path = LcLEditorUtilities.GetSelectionAssetPath();
            if (dir_path == "") return;
            string cmd = string.Format(GetCmdFormatString(), SVNCmd.REVERT, dir_path);
            Debug.Log(cmd);
            InvokeCmd(cmd);
        }
        // svn resolve
        [MenuItem("Assets/LcL SVN Operation/SVN Resolve...")]
        public static void Resolve()
        {
            var dir_path = LcLEditorUtilities.GetSelectionAssetPath();
            if (dir_path == "") return;
            string cmd = string.Format(GetCmdFormatString(), SVNCmd.RESOLVE, dir_path);
            Debug.Log(cmd);
            InvokeCmd(cmd);
        }
